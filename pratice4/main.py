...
def main():
    start = time.time()
    print("=== Génération des runs (exercice 4 + tuning BM25) ===")
    ...
    for stop_name, stopset in stop_options:
        for stem_name, stemmer in stem_options:
            ...
            for method in methods:
                if method == "bm25":
                    run_name = f"{run_id}_test_{method}_article_{stop_name}_{stem_name}_k{BM25_K1}_b{BM25_B}"
                    ...
                    run_name = f"{run_id}_test_{method}_article_{stop_name}_{stem_name}"
                    path, written, expected = generate_one_run(
                        run_name, method, postings, df, doc_len, doc_ids, N,
                        QUERIES, stopset, stemmer, stem_cache, OUTPUT_DIR
                    )
    ...
    # Exercice 6 : tuning BM25 (nostop / nostem)
    print("\n=== Exercice 6 : BM25 tuning (nostop / nostem) ===")
    ...
    for k1 in k1_values:
        for b in b_values:
            run_name = f"{run_id}_tune_bm25_article_{stop_name}_{stem_name}_k{k1}_b{b}"
            print(f"-> Génération run tuning {run_name} ...")
            path, written, expected = generate_one_run(
                run_name, "bm25", postings, df, doc_len, doc_ids, N,
                QUERIES, stopset, stemmer, stem_cache, OUTPUT_DIR,
                k1=k1, b=b
            )
            print(f"  Fichier : {path} — {written}/{expected}")
    ...
    run_name = f"{run_id}_tune_bm25_ref_article_{stop_name}_{stem_name}_k{1.2}_b{0.75}"
    print(f"-> Génération run référence {run_name} ...")
    path, written, expected = generate_one_run(
        run_name, "bm25", postings, df, doc_len, doc_ids, N,
        QUERIES, stopset, stemmer, stem_cache, OUTPUT_DIR,
        k1=1.2, b=0.75
